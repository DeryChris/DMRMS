...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-8 mb-6">
        <div class="flex items-center justify-between mb-6">
            ...
            <span class="text-xs font-semibold px-3 py-1 rounded-full {{ $appointment->status === 'scheduled' ? 'bg-blue-100 text-blue-700' : ($appointment->status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700') }}">{{ ucfirst($appointment->status) }}</span>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="flex items-start space-x-3">
                ...
                <div><p class="text-sm font-medium text-gray-800">Date</p><p class="text-sm text-gray-500">{{ $appointment->appointment_date?->format('F j, Y') ?? 'TBD' }}</p></div>
            </div>
            <div class="flex items-start space-x-3">
                ...
                <div><p class="text-sm font-medium text-gray-800">Time</p><p class="text-sm text-gray-500">{{ $appointment->slot?->start_time ? $appointment->slot->start_time . ' - ' . $appointment->slot->end_time : ($appointment->appointment_time ?? 'TBD') }}</p></div>
            </div>
            <div class="flex items-start space-x-3">
            ...
            <div class="flex items-start space-x-3">
                ...
                <div><p class="text-sm font-medium text-gray-800">Slot Reference</p><p class="text-sm text-gray-500">{{ $appointment->slot?->reference ?? 'SLOT-' . str_pad($appointment->id, 4, '0', STR_PAD_LEFT) }}</p></div>
            </div>
        </div>
    </div>
    @else
