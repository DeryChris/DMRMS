<x-guest-layout title="Admin — Login" subtitle="Sign in to your admin account">
    <form method="POST" action="{{ route('login') }}" style="display:flex;flex-direction:column;gap:18px;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;">
            ...
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" style="font-size:13px;color:#5fa489;font-weight:600;text-decoration:none;transition:color 0.2s;"
                   onmouseover="this.style.color='#4d8870'" onmouseout="this.style.color='#5fa489'">Forgot Password?</a>
            @endif
        </div>
        ...
        <p style="text-align:center;font-size:12px;color:#94a3b8;margin:0;border-top:1px solid #e2e8f0;padding-top:12px;">
            <a href="{{ route('applicant.login') }}" style="color:#64748b;text-decoration:none;transition:color 0.2s;"
               onmouseover="this.style.color='#14532d'" onmouseout="this.style.color='#64748b'">Applicant Portal</a>
        </p>
    </form>
</x-guest-layout>