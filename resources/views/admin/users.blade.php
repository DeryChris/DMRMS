...
<div x-data="{ showModal: false, editing: null, editId: null }" class="space-y-6">
    ...
    <div id="user-old-data" data-old="{{ json_encode(old()) }}" data-edit-id="{{ request()->route('user') ?? request('user') ?? '' }}" class="hidden"></div>
        ...
        <div class="glass-strong rounded-xl shadow-lg w-full max-w-md mx-4 p-8" style="background:rgba(255,255,255,0.95);backdrop-filter:blur(16px);">
            ...
            <form x-show="!editing && !editId" method="POST" action="{{ route('admin.users.store') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">First Name <span class="text-red-500">*</span></label><input type="text" name="first_name" value="{{ old('first_name') }}" oninput="this.value = this.value.replace(/[^A-Za-zÀ-ÖØ-öø-ÿ\s'\-]/g, '')" pattern="[A-Za-zÀ-ÖØ-öø-ÿ\s'\-]+" title="Only letters, spaces, hyphens, and apostrophes" class="w-full border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-gaf-khaki {{ $errors->has('first_name') ? 'border-red-500' : 'border-gray-300' }}" required>@error('first_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror</div>
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Last Name <span class="text-red-500">*</span></label><input type="text" name="last_name" value="{{ old('last_name') }}" oninput="this.value = this.value.replace(/[^A-Za-zÀ-ÖØ-öø-ÿ\s'\-]/g, '')" pattern="[A-Za-zÀ-ÖØ-öø-ÿ\s'\-]+" title="Only letters, spaces, hyphens, and apostrophes" class="w-full border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-gaf-khaki {{ $errors->has('last_name') ? 'border-red-500' : 'border-gray-300' }}" required>@error('last_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror</div>
                </div>
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1">Password <span class="text-red-500">*</span></label><input type="password" name="password" class="w-full border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-gaf-khaki {{ $errors->has('password') ? 'border-red-500' : 'border-gray-300' }}" required>@error('password') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror</div>
                <div><label class="block text-sm font-medium text-gray-700 mb-1">Role <span class="text-gray-400 font-normal">(Optional)</span></label>
            ...
            <template x-if="editId">
                <form method="POST" :action="`{{ url('admin/users') }}/${editId}`" class="space-y-4">
                    @csrf @method('PUT')
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">First Name <span class="text-red-500">*</span></label><input type="text" :id="`edit-first_name-${editId}`" name="first_name" oninput="this.value = this.value.replace(/[^A-Za-zÀ-ÖØ-öø-ÿ\s'\-]/g, '')" pattern="[A-Za-zÀ-ÖØ-öø-ÿ\s'\-]+" title="Only letters, spaces, hyphens, and apostrophes" class="w-full border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-gaf-khaki {{ $errors->has('first_name') ? 'border-red-500' : 'border-gray-300' }}" required>@error('first_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror</div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Last Name <span class="text-red-500">*</span></label><input type="text" :id="`edit-last_name-${editId}`" name="last_name" oninput="this.value = this.value.replace(/[^A-Za-zÀ-ÖØ-öø-ÿ\s'\-]/g, '')" pattern="[A-Za-zÀ-ÖØ-öø-ÿ\s'\-]+" title="Only letters, spaces, hyphens, and apostrophes" class="w-full border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-gaf-khaki {{ $errors->has('last_name') ? 'border-red-500' : 'border-gray-300' }}" required>@error('last_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror</div>
                    </div>
