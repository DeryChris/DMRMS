            ...
            <div class="glass-strong rounded-xl shadow-sm p-6 gradient-border-left">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="{{ $f }}" value="{{ old($f, $settings['contact_phone']) }}" maxlength="10" oninput="this.value=this.value.replace(/\D/g,'')" class="w-full px-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-gaf-khaki {{ $errors->has($f) ? 'border-red-500' : 'border-gray-300' }}">
                        @error($f) <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
