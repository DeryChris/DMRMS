...
@section('content')
<div x-data="{ sidebarOpen: true }" class="flex min-h-screen" style="background:linear-gradient(180deg, #f0f7f0 0%, #f8faf8 50%, #f0f7f0 100%);">
    @php $applicant = auth('applicant')->user(); @endphp
    <aside x-show="sidebarOpen" x-cloak class="w-64 bg-white shadow-lg fixed md:relative z-30 h-full md:h-auto gradient-border-right" x-transition style="border-right:1px solid rgba(20,83,45,0.1);">
    ...
    <div class="flex-1 flex flex-col min-h-screen">
        <header class="px-6 py-3 flex items-center justify-between gradient-border" style="background:linear-gradient(135deg, #f8faf8 0%, #ffffff 100%);">
            <div class="flex items-center space-x-3">
                <button @click="sidebarOpen = !sidebarOpen" class="text-gray-500 hover:text-gray-700">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
                </button>
                @unless(request()->routeIs('applicant.dashboard'))
                <a href="{{ route('applicant.dashboard') }}" class="inline-flex items-center space-x-1.5 text-sm text-gaf-green hover:text-gaf-dark-green font-medium transition">
