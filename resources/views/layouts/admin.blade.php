...
<body class="font-sans antialiased bg-gray-50 dark:bg-slate-900 dark:text-slate-100">
<div x-data="{ sidebarCollapsed: false, mobileSidebarOpen: false }" class="flex h-screen overflow-hidden">
    <div x-show="mobileSidebarOpen" @click="mobileSidebarOpen = false" x-cloak x-transition:enter="transition-opacity ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black/30 z-30 lg:hidden"></div>
    <aside x-bind:class="(sidebarCollapsed ? 'w-16' : 'w-64') + (mobileSidebarOpen ? ' translate-x-0' : ' -translate-x-full lg:translate-x-0')" class="bg-gaf-dark-green dark:bg-slate-950 text-white flex-shrink-0 transition-all duration-300 flex flex-col shadow-xl fixed inset-y-0 left-0 z-40 lg:relative lg:z-auto h-full" x-cloak>
        ...
        <div class="h-16 flex items-center justify-between px-4 border-b border-white/10 flex-shrink-0 relative z-10">
            ...
            <button @click="if (window.innerWidth < 1024) mobileSidebarOpen = false; else sidebarCollapsed = !sidebarCollapsed" class="text-white/40 hover:text-white p-1.5 rounded-lg hover:bg-white/10 transition-all">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"/></svg>
            </button>
        </div>
        <nav @click="if (window.innerWidth < 1024 && $event.target.closest('a, button')) mobileSidebarOpen = false" class="flex-1 overflow-y-auto p-3 space-y-0.5 min-h-0 scrollbar-hide relative z-10">
    ...
    <div class="flex-1 flex flex-col overflow-hidden min-w-0">
        <header class="bg-white dark:bg-slate-800 shadow-sm px-6 py-3 flex items-center justify-between flex-shrink-0 sticky top-0 z-30 gradient-border" style="background:linear-gradient(135deg, #f8faf8 0%, #ffffff 100%);">
            <div class="flex items-center space-x-4 flex-1 min-w-0">
                <button @click="if (window.innerWidth < 1024) mobileSidebarOpen = !mobileSidebarOpen; else sidebarCollapsed = !sidebarCollapsed" class="lg:hidden text-gray-400 hover:text-gray-600 p-1">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
                </button>
                <div class="relative max-w-md w-full">
                    <svg class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                    <input type="text" placeholder="Search applicants..." class="w-full pl-10 pr-4 py-2 bg-gray-50 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-gaf-khaki/30 focus:border-gaf-khaki focus:bg-white dark:focus:bg-slate-800 transition-all outline-none">
                </div>
            </div>
            <div class="flex items-center space-x-2 flex-shrink-0">
                ...
                <button @click="open = !open" class="flex items-center space-x-3 text-sm text-gray-700 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 transition-all">
                ...
                <div x-show="open" @click.outside="open = false" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-100 dark:border-slate-700 py-1.5 z-50">
                    <div class="px-4 py-2 border-b border-gray-100 dark:border-slate-700">
                        <p class="text-sm font-medium text-gray-900 dark:text-slate-100">{{ auth()->user()->name ?? 'Admin' }}</p>
                        <p class="text-xs text-gray-500 dark:text-slate-400">{{ auth()->user()->email ?? '' }}</p>
                    </div>
                    <a href="{{ route('admin.profile') }}" class="flex items-center space-x-2 px-4 py-2 text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition">
        ...
        <main class="flex-1 overflow-y-auto min-h-0 relative dark:bg-slate-900" style="background:linear-gradient(180deg, #f0f7f0 0%, #f8faf8 50%, #f0f7f0 100%);">
