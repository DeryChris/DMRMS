...
<div x-data="{
    ...
    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-2" class="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-2xl border border-gray-100 z-50 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100">
            <div class="flex items-center space-x-3">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-heading font-bold text-gray-900 truncate">{{ $applicant->name ?? 'Applicant' }}</p>
        ...
        <div class="px-5 py-3 space-y-2 text-sm">
            <div class="flex justify-between">
                <span class="text-gray-500">GAF ID</span>
                <span class="font-medium text-gray-800">{{ optional($applicant->application)->gaf_id ?? '—' }}</span>
            </div>
            <div class="flex justify-between">
                <span class="text-gray-500">Phone</span>
                <span class="font-medium text-gray-800">{{ $applicant->contact_number ?? '—' }}</span>
            </div>
            <div class="flex justify-between">
